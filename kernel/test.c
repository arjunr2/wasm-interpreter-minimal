...
#define OK_poly(type, fmt, decode, len, x) do {				\
    ...
    if (gotlen < 0) {							\
      pr_info("expected %d, but failed with %zd @ %s:%d\n", len, gotlen, __FILE__, __LINE__); \
      return 0;								\
    }									\
    if (gotval != val) {						\
      pr_info("expected " fmt ", but got " fmt " @ %s:%d\n", val, gotval, __FILE__, __LINE__); \
      return 0;								\
    }									\
  } while(0) 
...
#define OK_i64(len, val) OK_poly(int64_t, "%lld", decode_i64leb, len, val);
#define OK_u64(len, val) OK_poly(uint64_t, "%llu", decode_u64leb, len, val);

#define ERR_poly(type, fmt, decode) do {				\
    ssize_t gotlen = 0;							\
    type gotval = decode(bytes, bytes + sizeof(bytes), &gotlen);	\
    if (gotlen > 0) {							\
      pr_info("expected fail, but got val=" fmt ", len=%zd @ %s:%d\n", gotval, gotlen, __FILE__, __LINE__); \
      return 0;								\
    }									\
  } while(0)
...
#define ERR_i64 ERR_poly(int64_t, "%lld", decode_i64leb);
#define ERR_u64 ERR_poly(uint64_t, "%llu", decode_u64leb);
